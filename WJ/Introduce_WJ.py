...
# 부제목
st.header("일상을 게임처럼! 우리가 만드는 새로운 세상")

# 텍스트
st.write("예비창업자 소우주 입니다.")


# 경고 메시지
st.warning(" 재미 없는 지루한 일상! ")

# 에러 메시지
st.error("❌ 누구나 한번 쯤 게임으로 도피한 경험이 있습니다. ")

# 정보 메시지
st.info("ℹ️ 하기싫은 일이 게임 속 퀘스트가 된다면 어떨까요? ")

# 성공 메시지
st.success("✅ 유니워스는 당신의 세상이 재미있게 성장하도록 새롭게 만들어 줍니다. ")
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["BurningBunny", "젤리방어대", "Goetia", "엘라의 꿈", "냥융합", "뱀부 루틴"])
with tab1:
    st.write("리라몰 모바일 3D호러, GooglePlayStore 출시")
with tab2:
    st.write("게임톤 5기 수상작, GooglePlayStore 출시")
with tab3:
    st.write("공감오래 여름학기 계약직, Steam 게임 출시")
with tab4:
    st.write("하이퍼 비주얼 노벨, 졸업작품")
with tab5:
    st.write("모바일 퍼즐게임, 제작 진행중")
with tab6:
    st.write("루틴앱, 제작 진행중")
